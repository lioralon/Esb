...
namespace ESBasic.Collections
{
    ...
    public class SortedArray<TKey ,TVal> where TKey : IComparable
    {
        ...
        public SortedArray(int capacity) 
        {
            if (capacity > 0)
            {
                this.keyArray = new TKey[capacity];
                this.valArray = new TVal[capacity];
            }
        }
        ...
        /// </summary>       
        public void GetByKeyScope(TKey minKey, TKey maxKey, out TKey[] resultKey, out TVal[] resultVal) //闭集
        {
            using (this.smartRWLocker.Lock(AccessMode.Read))
            {
                ...
                minIndex = hitMin ? minIndex : minIndex + 1;
        ...
        /// </summary>
        public TVal[] GetByKeyScope(TKey minKey, TKey maxKey)
        {
            TKey[] resultKey = null;
            TVal[] resultVal = null;

            this.GetByKeyScope(minKey, maxKey, out resultKey, out resultVal);
