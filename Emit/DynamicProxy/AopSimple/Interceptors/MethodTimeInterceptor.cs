...
namespace ESBasic.Emit.DynamicProxy.AopSimple
{
    ...
    public sealed class TimeLogArounder : IArounder
    {
        ...
        {
            this.stopwatch.Stop();
            string methodPath = this.interceptedMethod.Target.GetType().ToString() + "." + this.interceptedMethod.MethodName;
            this.methodTimeLogger.Log(methodPath, this.stopwatch.ElapsedMilliseconds);
