        ...
        protected override bool DoDetect()
        {
            try
            {
                lock (this.locker)
                {
                    ...
                    bool isLastPhaseOfRound = this.phaseIncreaseAccesser.NextIsLastPhaseOfRound(out currentRoundID, out lastKeyOfRoundDic);

                    if (!isLastPhaseOfRound)
                    {
                        foreach (TSourceToken token in sourceList)
                        {
                            TKey maxKeyOfThisPhase = this.phaseIncreaseAccesser.GetMaxKey(token);
